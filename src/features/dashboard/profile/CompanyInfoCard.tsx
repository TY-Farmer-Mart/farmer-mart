...
export const CompanyInfoCard = () => {
  ...
  useEffect(() => {
    const fetchCompany = async () => {
      try {
        ...
        const userId = storedUser?.user?.id; // ✅ Correct nested path

        if (!userId) {
          console.error("User ID not found in localStorage object:", storedUser);
          return;
        }

        const res = await getCompanies(userId); // ✅ GET call using userId
        if (res && res.company) {
  ...
  const handleUpdate = async () => {
    try {
      ...
      await updateCompany(payload); // ✅ PUT call
    ...
    <div className="relative bg-white rounded-lg shadow-md px-3 py-4 mt-4">
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4">
          ...
          <Input
            label={PROFILE_PAGE_TXT.COMPANY_TYPE}
            name="companyType"
            className="px-4 py-2"
