...
const Navbar: FC<NavbarProps> = ({
  ...
  const handleClick = () => {
    navigate(`/products?product=${product}&location=${currentState}`);
  };
  ...
  return (
    <nav className="w-full shadow border border-1 bg-blue-900 px-4 sm:px-6 py-3 sticky top-0 z-50">
        ...
        <div className="flex items-center space-x-4">
          <div className="flex-shrink-0">
            <img
              src={logo}
              alt="logo"
              className="w-40 h-16 sm:w-48 sm:h-15"
            />
          </div>

          {/* Desktop/Tablet: Location + Search */}
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                ...
                <span className="flex-1 text-center text-sm sm:text-base md:text-base mx-2 truncate">
                  {stateOptions.find((opt) => opt.value === currentState)
                    ?.label || currentState}
                </span>
              ...
              {dropdownOpen && (
                <div className="absolute top-full left-0 mt-1 w-[180px] bg-white border rounded-md shadow-lg z-10 p-2">
      ...
      {/* MOBILE: Location + Product Inputs */}
      <div className="sm:hidden mt-2 px-4 space-y-2">
        ...
        <div className="relative" ref={dropdownRef}>
          <Input
            ...
            value={
              searchText ||
              stateOptions.find((opt) => opt.value === currentState)?.label ||
              ""
            }
          ...
          {dropdownOpen && (
            <ul className="absolute top-full left-0 mt-1 w-full bg-white border rounded-md shadow-lg z-10 max-h-40 overflow-y-auto">
              {stateOptions
                .filter((opt) =>
